classdef DotInfo3DClass < handle
    properties
        %         numDotField = 1;
        apXYD = [0 0 50];
        speed = [50];
...
%         fixColor = [0 255 255];
        ...
        cohSetColor = [0.2,0.4,0.6,0.8] % p blue % ToDo
        cohSetSize = [0.2,0.4,0.6,0.8] % p big % ToDo
...
%             obj.sampleDuration();
        ...
        function sampleRandomCoh(obj)
            ...
            ind = ceil(rand*length(obj.cohSetColor));
            obj.pDotColor2 = obj.cohSetColor(ind);
            
            ind = ceil(rand*length(obj.cohSetSize));
            obj.pDotLarge = obj.cohSetSize(ind);
        end
...
%         end
        ...
        function ForTrialData = trialInfo(obj)
            ...
            ForTrialData.dots_color     = obj.dots_color;
            ForTrialData.dots_size     = obj.dots_size;
            ForTrialData.rseed      = obj.rseed;
            ForTrialData.coh        = obj.coh;
            ForTrialData.coh_std    = obj.coh_std;
            
            ForTrialData.coh         = obj.coh;
            ForTrialData.pDotLarge   = obj.pDotLarge;
            ForTrialData.pDotColor2  = obj.pDotColor2;
...
%             Screen('DrawingFinished',curWindow,dontclear);
            ...
            obj.vars_for_draw = struct('Ls',Ls,'ss',ss,'loopi',loopi,'coh',coh,'coh_std',coh_std,...
                ...
                'center',center,'rotationMat',rotationMat);
            obj.puntos    = [];
            obj.cohframes = [];
            obj.ncohdots  = [];
            obj.dots_size = [];
...
%             Screen('BlendFunction', curWindow, GL_DST_ALPHA, GL_ONE_MINUS_DST_ALPHA);
            ...
            % 3D: colors and sizes
            nn = size(dot_show,2);
            dotSize = obj.dotSize + (rand(nn,1)<obj.pDotLarge)*obj.deltaDotSize;
            
            dotColorId = ones(nn,1);
            I = rand(nn,1)<obj.pDotColor2;
            dotColorId(I) = 2;
            dotColor = obj.dotColors(dotColorId,:);
...
%             obj.puntos{obj.frames} = dot_show_aux;
            ...
            obj.dots_color{obj.frames} = dotColorId;
            obj.dots_size{obj.frames} = dotSize;
